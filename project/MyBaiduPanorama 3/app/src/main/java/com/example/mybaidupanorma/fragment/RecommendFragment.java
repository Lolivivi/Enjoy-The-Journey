...
public class RecommendFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        smartRefreshLayout = view.findViewById(R.id.refresh_recommend);
        ...
        new Thread(){
            ...
            public void run() {
                ...
                try {
                    ...
                    url = new URL(ConfigUtil.BASE_URL+"note/recommendInterface");
                    HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                    conn.setRequestMethod("POST");
                    OutputStream out = conn.getOutputStream();
                    out.write(("tel=1&count="+page).getBytes());
                    page+=5;
                    ...
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            RecommendAdapter adapter = new RecommendAdapter(mContext,users);
